...
def disable(user, p_id):
    try:
        ...
        if status != 'closed':
            return (400, "This event is not closed.")

        if user != poster:
            return (400, "Only poster can disable the event.")

        sql2 = f"""UPDATE post SET p_status = 'disable' WHERE p_id = {p_id};"""
        execute(sql2)

        result = (200, "Disable Successed.")

    except Exception as e:
        print("Error : ", e)
        result = (400, "Disable failed.")
